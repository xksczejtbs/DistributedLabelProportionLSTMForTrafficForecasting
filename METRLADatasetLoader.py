...
class METRLADatasetLoader(object):
    ...
    def _read_web_data(self):
        ...
        if not os.path.isfile(self.data_statistics_path + '/Static_node_Features_mins_METRLA.pt'):
            with FileLock(os.path.expanduser("~/.data.lock")):
                torch.save(torch.from_numpy(mins), self.data_statistics_path + '/Static_node_Features_mins_METRLA.pt')
                torch.save(torch.from_numpy(maxs), self.data_statistics_path + '/Static_node_Features_maxs_METRLA.pt')
                torch.save(torch.from_numpy(means), self.data_statistics_path + '/Static_node_Features_means_METRLA.pt')
                torch.save(torch.from_numpy(stds), self.data_statistics_path + '/Static_node_Features_stds_METRLA.pt')
