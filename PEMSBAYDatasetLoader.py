...
class PemsBayDatasetLoader(object):
    ...
    def _read_web_data(self):
        ...
        X = np.delete(X,1,1) #delete uninformative time points
        ...
        if not os.path.isfile(self.data_statistics_path + '/Static_node_Features_mins_PemsBay.pt'):
            with FileLock(os.path.expanduser("~/.data.lock")):
                torch.save(torch.from_numpy(mins), self.data_statistics_path+'/Static_node_Features_mins_PemsBay.pt')
                torch.save(torch.from_numpy(maxs), self.data_statistics_path + '/Static_node_Features_maxs_PemsBay.pt')
                torch.save(torch.from_numpy(means), self.data_statistics_path + '/Static_node_Features_means_PemsBay.pt')
                torch.save(torch.from_numpy(stds), self.data_statistics_path + '/Static_node_Features_stds_PemsBay.pt')
